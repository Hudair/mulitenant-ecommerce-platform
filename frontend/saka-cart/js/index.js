(function ($) {
  ...
  function get_data() {
      $.ajax({
            ...
            data:{latest_product:1,random_product:1,best_selling_product:1,trending_products:1,sliders:1,bump_adds:1,banner_adds:1,featured_brand:1,featured_category:1,menu_category:1,get_offerable_products:20},
            dataType: 'json',            
            beforeSend: function() {
                ...
                if(response.featured_category.length > 0){
                  $.each(response.featured_category, function(index, value){
                    var html ='<div class="u-s-m-b-30 pl-12">';
                        html +='<div class="promotion-o">';
                        html +='<div class="aspect aspect--bg-grey aspect--square">';
                        html +=' <img class="aspect__img lazy"  src="'+preloader+'" data-src="'+value.preview+'" alt=""></div>';
                        html +='<div class="promotion-o__content">';
                        html +='<a class="promotion-o__link btn--e-white-brand" href="'+base_url+'/category/'+value.slug+'/'+value.id+'">'+value.name+'</a></div></div></div>';
